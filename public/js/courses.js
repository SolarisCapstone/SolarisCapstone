  ...
  .addEventListener("change", async (e) => {
    ...
    if (data.requirements) {
      for (const key in data.requirements) {
        if (
          Array.isArray(data.requirements[key]) ||
          data.requirements[key]?.courses ||
          data.requirements[key]?.options
        ) {
    ...
    if (data.coreCourses)
      sections.push({ label: "Core Courses", courses: data.coreCourses });
    if (data.mathStatsCourses || data.math_and_stats || data.mathStats)
      sections.push({
        label: "Math and Stats",
        courses: data.mathStatsCourses || data.math_and_stats || data.mathStats,
      });

    if (data.capstone || data.capstoneCourses || data.capstoneOptions)
      sections.push({
        label: "Capstone",
        courses: data.capstoneCourses || data.capstoneOptions || data.capstone,
      });
    ...
    if (data.concentrationRequirements) {
      if (data.concentrationRequirements.requiredCourses)
        sections.push({
          ...
          courses: data.concentrationRequirements.requiredCourses,
        });
      if (data.concentrationRequirements.technicalElectives)
        sections.push({
          label: "Concentration Technical Electives",
          note:
            data.concentrationRequirements.technicalElectives.options ||
            data.concentrationRequirements.technicalElectives,
        });
    }
